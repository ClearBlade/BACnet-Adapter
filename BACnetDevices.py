...
class BACnetDevices():
    ...
    def __init__(self, cb_client, bacnet_adapter, system: System):
        ...
        for device in devices:
            ...
            if device["device_name"] == None or device["device_name"] == "".encode('utf-8'):
                self._initialize_new_device(device["item_id"], device["ip_address"].encode('utf-8'))
    ...
    def _initialize_new_device(self, device_item_id, device_ip):
        print("BACnetDevices _initialize_new_device")
        self.bacnet_adapter.who_is(None, None, Address(device_ip))

    def got_new_device_who_is_response(self, apdu):
        print("BACnetDevices got_new_device_who_is_response")
    ...
    def _got_device_object_name(self, iocb, device_id):
        print("BACnetDevices _got_device_object_name")
        if iocb.ioError:
    ...
    def _get_new_sensors_for_new_device(self, source, device_id):
        print("BACnetDevices _get_new_sensors_for_new_device")
        request = ReadPropertyRequest(
            destination=source,
    ...
    def _got_sensors_for_device(self, iocb):
        print("BACnetDevices _got_sensors_for_device")
        if iocb.ioError:
